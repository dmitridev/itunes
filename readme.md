...
| gradlew       | CLI для gradle для командной строки. Например, используйте ``gradlew bootRun`` для запуска проекта.
...
| repository    | Пакет для интерфейсов, которые предоставляют набор методов для работы с бд. Spring автоматически реализует эти интерфейсы при старте приложения.<br>https://docs.spring.io/spring-data/jpa/docs/current/reference/html/#repositories |
